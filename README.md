...
- `demo_midday.py` — standalone demo proving out the "new agent joins
  mid-day" bonus feature end-to-end (see below).
- `data.json` — the sample dataset from the assignment brief.
- `test_cases/` — the 10 provided test-case JSON files (all pass).
- `test_case_reports/` — a generated `report.json` for each of the 10 test
  cases (already run for you).
- `sample_output/` — an example `report.json` and `top_performer.csv`
  generated from `data.json`, plus `midday_demo/` with the mid-day demo's
  output.
